...
- Revisions (09-25, colleague feedback on #42): widget settings labelled by the kind of test (RMET-like; hard ENEM forms from the guessing vignette); the Xiao et al. (2026) safety-valve mixture fitted to `roar_lexical` (π̂ 0.87; 9 of 11 heavy rapid responders in the guessing class; log likelihood +298 over Rasch vs +199 for the fixed floor, in-sample), problem 6 now asks whether it holds out of sample; `vocabulary_iq` blanks (coded 0 in the source, scored wrong in the IRW table) handled by setting aside the 78 respondents scoring 4 or less (2,724 kept): 3PL c median 0.036 (max 0.47) without them vs 0.000 with them, which is problem 4's live case; `expbeta` vs `beta` in mirt shown (plain beta puts every c at 0.50–0.51).
