...
- **Forms.** The WJ-III has parallel forms A and B with different words at the same positions. The first draft read the form from the Project KIDS total-scores file on LDbase; the IRW then republished the table (version 435, item_response_warehouse v61.0, irw PR #2437) with the form in the item names (`wj_lw_A_13s`; 228 items; `wj_lw_formunknown_<n>s` where no form was recorded, mostly project 3's spring). The lesson now reads the form from the item name and loads nothing outside the IRW; results are identical. It treats form × position as the item: 140 items with at least 100 responses. The other Project KIDS WJ wave tables still pool forms.
...
- **Citations:** Fischer (2017) removed from *Going further* (the chapter could not be verified on Crossref). Added and verified: van Dijk et al. (2022), Connor et al. (2007, doi:10.1126/science.1134513), Dumont & Willis (2008, doi:10.1002/9780470373699.speced2229) for the WJ-III, Warm (1989), Mislevy & Wu (1996). (The Project KIDS total-scores data citation was dropped with the LDbase read.)
- **Ben's review (09-25), second pass.** (1) The ACM Go deeper now has two small figures: single cancellation (arrows within rows and columns of a Rasch 3 × 3 grid) and double cancellation (premise and conclusion arrows, with a toggle to a 2PL grid where the conclusion fails). (2) The descriptive cancellation count was replaced by ConjointChecks (Domingue, 2014) on a complete block (form A, positions 2–40, records reaching 40; 3,538 records × 23 items), with a Rasch-generated copy of the block as baseline: share of cells outside 95% credible intervals 0.17 real vs 0.11 copy (double cancellation), 0.17 vs 0.10 with single cancellation added. Runs at render time (about a minute). Idea 5 ("What to do about it") was folded into the verdict to stay within 3,000 words.
