...
| `himmelstein-number_series-2025` | sanity | The pipeline reproduces the construct-map order found in `constructs` (NS_2 at 0.84, NS_6 at 0.015 with both waves pooled; NS_6 is hard partly because its key accepts 165 where the tripling rule gives 120: see `constructs`). | `constructs` |
