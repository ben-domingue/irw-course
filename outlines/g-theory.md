...
- **Revision after colleague feedback (#76, 09-25):** "what should the score generalize over?" is now the organizing question (universe of admissible observations; random vs. fixed facets). The G coefficients are framed as σ²_T/(σ²_T + σ²_E), recalling `ctt-reliability`. The ANOVA/EMS example in *With real data* is gone; the Go deeper keeps only the EMS line and the alpha proof. The lesson says why it uses `lmer` (REML removes ML's downward bias for facets with few levels; Patterson & Thompson 1971, Harville 1977), shown on the cleverness data (rater 0.0199 REML vs 0.0163 ML). Brennan's interrater coefficient with tasks fixed, (σ²_p + σ²_pt)/(σ²_p + σ²_pt + σ²_pr + σ²_ptr,e), is 0.64 on the cleverness data, against 0.33 when tasks are generalized over. The D-study section spells out the assumptions behind hypothetical raters, and the widget notes the observed design. Pointers now go to the new `rater-models` lesson.
