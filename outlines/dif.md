...
2. **DIF: the same item, conditional on the construct.** If group membership predicts a response at equal θ, the item functions differently (slides 17–21); uniform vs. non-uniform. The hard part is the x-axis (slide 20): matching on the rest score or θ from the other items; anchors, purification, and why DIF is always relative to the other items (slide 22). One paragraph recalls multigroup CFA for readers who have done `measurement-invariance` (loadings ↔ non-uniform, intercepts ↔ uniform DIF). *(major)* Sources: Lord (1980), doi:10.4324/9780203056615, ch. 14; Thissen, Steinberg & Wainer (1993), in Holland & Wainer (1993); Swaminathan & Rogers (1990), doi:10.1111/j.1745-3984.1990.tb00754.x.
...
- Multigroup CFA and invariance (from `measurement-invariance`, not an ancestor: one "if you've done" paragraph, E2).
