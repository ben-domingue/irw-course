...
- Unfolding response processes → `unfolding`.
...
- Settled (Ben, 09-24): `andrich_mudfold` returns in `unfolding` as a thread. Record it with `reuses:` once #73 merges.
- Other unfolding data in the IRW (checked 09-24), none of which replaces `andrich_mudfold` here, because none shows the single-peaked curve against a sum score:
  ...
  `eurpar2_mudfold` and `franco_2024_unfolding` suit `unfolding` (noted on #52).
