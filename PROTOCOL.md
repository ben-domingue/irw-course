...
| Across the IRW | **Deep-dive lessons only** (§5): the precomputed result across many IRW tables, with its code, then a pointer to the vignette. Omitted otherwise. |
...
- **At most 3** IRW tables per lesson, each with a job (main example, contrast or
  failure case) given in one sentence where the table is introduced. The ceiling
  governs worked examples: a deep dive's corpus and a sanity table (below) don't count
  against it (#4).
...
- A few lessons carry a **deep dive**: a result across the whole IRW corpus (#4).
  Candidates are #18–#26. It has three parts:
  1. a miniature on one table, in *With real data*;
  2. the precomputed corpus result with its code, in its own H2, **Across the IRW**,
     after *Ask Claude* and before *Going further*;
  3. a pointer to the vignette, at the end of that section and again in *Going further*.
- **Exception to the table ceiling.** The corpus doesn't count against the 3 tables.
- **Compute, then render (#4).** Borrowed from the IRW vignettes (`irw-vignette` skill,
  `dimensionality_compute.R`). The lesson never fits the corpus itself:
  - `deepdives/<lesson-id>/compute.R` is run by hand (it may need a Redivis token and
    hours) and writes one committed `deepdives/<lesson-id>/results.rds`. The render
    only reads that file, so it still needs no token.
  - The script has a fixed `set.seed()`; the `irw_filter()` call with the reason for
    each threshold in a comment; a `PILOT <- TRUE/FALSE` flag; each table's fit in
    `tryCatch`, so a failing table is logged and dropped; and one cache file per
    table (`deepdives/<lesson-id>/fits/<table>.rds`, gitignored), so an interrupted run
    resumes.
  - The pilot runs on 1–3 **known-good tables** (ones whose answer is obvious, e.g.
    `psychtools_bfi` for dimensionality) plus a small random top-up, so it checks the
    pipeline rather than only shrinking the sample.
  - `results.rds` holds at least `summary`, `candidate_tables`, `n_all_candidates`,
    `failed`, `pilot`, `date_run` and `session`.
  - The page reads `results.rds` in a folded chunk and links `compute.R`. It shows a
    *Results not computed* callout when the file is missing and a *Partial run: n of N
    tables* callout for a pilot, so a draft can go up for review before the full run
    finishes. It states the date of the run and how many tables failed.
- **Sanity tables.** Any lesson whose real-data section rests on a model fit first
  checks the pipeline on a table whose answer is known. Name it in the outline's
  Tables section. It needn't appear in the lesson and doesn't count against the
  ceiling.
...
- [ ] At most 3 tables, each job stated in one sentence; none reused outside a recorded
      thread. A deep dive's corpus and the sanity table don't count.
- [ ] Deep dive (if any): *Across the IRW* after *Ask Claude*; `compute.R` and a full
      (not pilot) `results.rds` committed; neither the missing-results nor the
      partial-run callout shows; run date and failed-table count stated.
